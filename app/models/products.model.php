...
class ProdModel{
    ...
    function update($id, $product, $category, $description){
        ...
        $query->execute(array($id, $product, $description, $category));
        var_dump($query->errorInfo());
    ...
    function filter($id_category){
        $query = $this->db->prepare('SELECT a.*, b.* FROM productos a INNER JOIN categorias b ON a. id_categoria = b. id_categoria WHERE id_categoria = ?');
